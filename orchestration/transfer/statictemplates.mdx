---
title: "Static template (recurring transfers)"
---
...
<Tip>
  \*\*Providing an amount is optional \*\*but allowed when creating a Static Template. If no amount is provided, the Transfer will automatically have the `flexible_amount` feature enabled as well.
</Tip>
...
You can query for transfer instances created from the static template using the [<u>Get all transfers</u>](https://apidocs.bridge.xyz/reference/get_transfers) endpoint.
...
- You can use the [<u>Update Transfer</u>](https://apidocs.bridge.xyz/reference/put_transfers-transferid) endpoint to make changes to the template. This will not change any existing instances of that template, but funds received after the changes will be processed according to the updated template instructions.
- You can use the `template_id` query parameter to [<u>List All Transfer instances</u>](https://apidocs.bridge.xyz/reference/get_transfers) created from this template.
