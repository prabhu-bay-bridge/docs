...
The below example uses Bridge's transfer API to fund the wallet. There are a variety of methods to fund the wallet using Bridge's orchestration APIs (virtual accounts, transfers, or liquidation addresses). Learn more at [Bridge Wallets](ref:bridge-wallets).
...
:credit_card:\*\* You can now share these card details with your customer to start spending\!\*\*
...
Or you can fetch transactions manually via the [Retrieve card transactions](ref:get_customers-customerid-card-accounts-cardaccountid-transactions) API.
...
Check out the [Cards](doc:cards) section for more in depth details\!