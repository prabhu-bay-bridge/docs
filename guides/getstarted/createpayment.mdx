...
- At least one verified customer - see [Create your first customer](doc:create-your-first-customer).
...
For detailed information, see [Transfers](doc:transfers-2).
...
When Bridge receives the deposit, we'll transition the transfer state to `funds_received`. From there, you can monitor the state until it's finally delivered. The transfer status when it's delivered is `payment_processed`. You can view the full breakdown of transfer states at [Transfers](ref:transfers).

## Fetching Transfers status via API

You can use the [Get a transfer](ref:get_transfers-transferid) endpoint to fetch the latest Transfer status.
...
```json Payment processed example
{
  "count": 2,
  "data": [
    {
...
You can set up webhooks to receive real-time notifications about transfer status changes. See [guide](https://apidocs.bridge.xyz/docs/setting-up-webhooks) for more details on setting up webhooks.
...
- [Make recurring payments using saved payment routes](doc:make-recurring-payments-using-saved-payment-routes)
