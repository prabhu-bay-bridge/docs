...
If [Customers API](/platform/customers/customers/api) and [KYC Links](/platform/customers/customers/kyclinks) return that the customer's KYC status is `rejected`, there are two fields shared with rejection reasons:
