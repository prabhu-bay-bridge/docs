...
An alternative to using the [Customers API](/platform/customers/customers/api) to KYC/B Customers (both individuals and businesses) is to use KYC Links. KYC Links support individual and business customers from all over the world (US \+ international).
...
For reference, see our page on [Rejection Reasons](/platform/customers/customers/rejection_reasons) for `developer_reason` and `reason` mappings for KYC rejection reasons.