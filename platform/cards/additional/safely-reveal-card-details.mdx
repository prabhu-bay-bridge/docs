...
This ephemeral key can only be used once, so the client will need to generate a new secret and nonce in order to show the card details again. In order to comply with [PCI DSS requirement 3.4](https://listings.pcisecuritystandards.org/documents/PCIDSS_QRGv3_1.pdf), you must not store the card details returned from this endpoint.