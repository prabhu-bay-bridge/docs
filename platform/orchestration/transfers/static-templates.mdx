...
--data-raw '{
  ...
  "features": {
	...
	"allow_any_from_address": true // lets the deposit originate from any address
  }
}'
```
...
You can query for transfer instances created from the static template using the [Get all transfers](/api-reference/transfers/get-all-transfers) endpoint.
...
- You can use the `template_id` query parameter to [List All Transfer instances](/api-reference/transfers/get-all-transfers) created from this template.
