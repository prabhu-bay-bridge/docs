...
If [<u>Customers API</u>](/customers/api) and [<u>KYC Links</u>](/customers/kyclinks) return that the customer's KYC status is `rejected`, there are two fields shared with rejection reasons:
