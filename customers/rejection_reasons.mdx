...
If [Customers API](/customers/api) and [KYC Links](/customers/kyclinks) return that the customer's KYC status is `rejected`, there are two fields shared with rejection reasons:
